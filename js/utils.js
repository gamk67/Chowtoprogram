(function() {
  ...
  SlideShow.prototype = {
    ...
    _update: function(targetId, dontPush) {
      ...
      if (takingnotes) { // update the note-taking area      
	var area = query('#ta');
	// store the old one, get the new one
	window.localStorage.setItem(window.location.hash, area.value);
	area.value = window.localStorage.getItem('#' + this.current);
	//area.value = this.current; // debugging....
      }
    ...
    toggleTakenotes: function() {
      this._tknotes.style.display = "block";
      this._tknotes.classList.toggle('invisible');
	// get the id of the current page
        // note that this can change behind the scenes (arrow pressed when notes are shown)
      var key = window.location.hash;
      var area = query('#ta');
      if (!takingnotes) { 
	takingnotes = true;
	// id used as local storage key in get/setitem
	area.value = window.localStorage.getItem(key);
      } else { 
	takingnotes = false;
	window.localStorage.setItem(key, area.value);
	// id used as local storage key in setitem
      }
    },
    viewSource: function() {
...
//  query('#toc-list').innerHTML = li_array.join('');
  ...
  SlideShow.prototype.changeTheme();   //AMK - prefer the 3D one
})();
